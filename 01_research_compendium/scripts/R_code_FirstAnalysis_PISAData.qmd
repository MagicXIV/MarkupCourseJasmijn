...
# R CODE
### *Of a small scale intermediate study to investigate the impact of motivation on the results of students partaking in the PISA test.*
...
# Package names
packages <- c("tidyverse", "haven", "dplyr", "tidyr", "lavaan", "lavaanPlot", "shiny", "ggplot2", "DiagrammeR", "DiagrammeRsvg", "rsvg")
...
If you want, you can run the code below and play around with the bin-width of this plot, and the previous plot on missing items per person, in a shiny-app: 
```{r}
ui1 <- fluidPage(
...
# Plot the essence of the SEM model
EmptyPlot <- lavaanPlot(
    model = NLfit,
    coefs = FALSE,          # Do not show path coefficients
    stand = FALSE,          # Do not use standardized coefficients
    stars = NULL,           # No significance stars
    covs = FALSE            # Suppress covariance paths
)
...
#plot the SEM model on the data of the Dutch students
NLPlot <- lavaanPlot(
  model = NLfit,
...
#plot the SEM model on the data of the German students
DEPlot <- lavaanPlot(
  model = DEfit,
  coefs = TRUE,      # Show path coefficients
  stand = TRUE,      # Standardized coefficients
...
If you want, you can save the plots by running the next code chunk. Please note: they are already in the results folder as well:
```{r}
# Function to fit the SEM model, generate the plot, and save it
save_sem_plots <- function(fit, file_name) {
  sem_plot <- lavaanPlot(
    model = fit,
    coefs = TRUE,      # Show path coefficients
    stand = TRUE,      # Standardized coefficients
  ...
  # Save as SVG and convert to TIFF
  svg_file <- tempfile(fileext = ".svg")
  export_svg(sem_plot) %>% writeLines(svg_file)
  rsvg_png(svg_file, file_name) 
}

# Function to save an existing plot as a PNG file
save_existing_plot <- function(plot, file_name) {
  # Export the plot to an SVG format as a string
  svg_file <- tempfile(fileext = ".svg")
  export_svg(plot) %>% writeLines(svg_file)
  
  # Convert the SVG to a PNG file
  rsvg_png(svg_file, file_name)
  
  # Optional: Inform user where the file is saved
  cat("File saved to:", normalizePath(file_name), "\n")
}

# Save the plots
save_sem_plots(NLfit, "../results/SEMModelNL.png")
save_sem_plots(DEfit, "../results/SEMModelDE.png")
```


```{r}
# Function to generate lavaanPlot
generate_sem_plot <- function(fit_object) {
  lavaanPlot(
  model = fit_object,
  coefs = TRUE,      # Show path coefficients
  stand = TRUE,      # Standardized coefficients
...
# Define UI
ui2 <- fluidPage(
  titlePanel("SEM Model Viewer"),
  sidebarLayout(
    sidebarPanel(
      selectInput("dataset", "Select Dataset:", 
                  choices = c("Dutch (NL)", "German (DE)"))
    ),
    mainPanel(
      plotOutput("semPlot")
    )
...
server2 <- function(input, output) {
  output$semPlot <- renderPlot({
    ...
    } else if (input$dataset == "German (DE)") {
      lavaanPlot(
        model = DEfit,
        coefs = TRUE,      # Show path coefficients
        stand = TRUE,      # Standardized coefficients
